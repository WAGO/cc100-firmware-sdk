...
function reformat_emmc_gpt
{
    ...
    if [[ -e $emmc_card_device ]]; then
        ...
        disable_automount && \
        sgdisk --zap-all ${emmc_card_device} && \
        sgdisk --resize-table=128 -a 1 \
        -n  1:34:5153         -c 1:ssbl \
        -n  2:5154:37921      -c 2:BOOT -t 2:0700 \
        -n  3:37922:54305     -c 3:log_backup \
        -n  4:54306:70689     -c 4:settings \
        -n  5:70690:1313825   -c 5:rootfs.1 \
        -n  6:1313826:3689505 -c 6:home1 \
        -n  7:3689506:6065185 -c 7:home2 \
        -n  8:6065186:7308321 -c 8:rootfs.2 \
        -p ${emmc_card_device} && \
        ...
        mkfs.ext4 -F -L "rootfs.1" ${emmc_card_device}p5 && \
...
function create_filesystems_emmc_gpt
{
    local index=0
    local pnum=2
    local device
...
function mount_target_emmc_gpt
{
    local -a partition_mount_points=("${!1}") #partition labels
    local -a partition_fs_types=("${!2}")     #vfat, ext4, " " , ...
...
function copy_sd_to_emmc
{
    ...
    print_dbg "    processing /boot/loader..."
    ${BACKUP_CMD} /boot/loader/*        ${DEST_BOOT}/
    local dev="$(GetTargetName)"
    cp "${DEST_BOOT}/mlo.${dev}" ${DEST_BOOT}/mlo

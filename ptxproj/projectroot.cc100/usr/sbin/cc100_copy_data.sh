...
function mount_target_sd
{
    ...
    do_mount ext4 ${G_SD_ROOT_DEV} "/tmp/sd/root"
}
    ...
    if [[ -e ${sd_card_device} ]]; then
       ...
       if is_in_use "$sd_card_device"; then
        ...
        if [[ "${sd_card_size}" != "" ]]; then
            ...
            do_mkfs "${sd_card_device}p5" "ROOT" "ext4"
            return $SUCCESS
        fi
    fi
    return $INVALID_PARAMETER
...
function check_size
{
    ...
    if [[ ${size_arg} -gt ${sd_card_size} ]]; then
        false
    else
        true
    fi
...
case ${target} in
    ...
    ${INTERNAL_FLASH_EMMC})
        reformat_emmc_gpt && \
        mount_target_emmc_gpt "EMMC_PARTITION_LABELS[@]" "EMMC_PARTITION_FS_TYPES[@]" && \
        copy_sd_to_emmc && \
        #TODO
        set_boot_partition_to_first
        status=$?

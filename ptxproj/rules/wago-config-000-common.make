...
ifdef PTXCONF_CT_AUTO_FIRMWARE_RESTORE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/auto_firmware_restore);
endif

ifdef PTXCONF_CT_CF_CARD_BACKUP
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/cf_card_backup);
endif

ifdef PTXCONF_CT_CHANGE_BOOTFLAG
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/change_bootflag);
endif
...
ifdef PTXCONF_CT_CHANGE_HOSTNAME
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/change_hostname);
	@$(call install_alternative, config-tools, 0, 0, 0640, /etc/specific/host.conf)
endif

ifdef PTXCONF_CT_CHANGE_KEYBOARD_LAYOUT
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/change_keyboard_layout);
endif

ifdef PTXCONF_CT_CHANGE_RTS_CONFIG
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/change_rts_config);
endif

ifdef PTXCONF_CT_CONFIG_CODESYS3
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_codesys3);
endif

ifdef PTXCONF_CT_CHANGE_SCREEN_STATE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/change_screen_state);
endif

ifdef PTXCONF_CT_CHANGE_VIDEO_MODE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/change_video_mode);
endif

ifdef PTXCONF_CT_CODESYS_WEBSERVER_WATCHDOG
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/codesys_webserver_watchdog);
endif

ifdef PTXCONF_CT_CONFIG_CLOCK
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_clock);
endif

ifdef PTXCONF_CT_CONFIG_CLOCK_DISPLAY_MODE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_clock_display_mode);
endif

ifdef PTXCONF_CT_CONFIG_DEFAULT_GATEWAY
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_default_gateway);
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_default_gateway_config);
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/events/networking/update_config);
endif

ifdef PTXCONF_CT_CONFIG_OVERWRITE_DHCP_GATEWAY
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/set_overwrite_dhcp_gateway);
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_overwrite_dhcp_gateway);
	@$(call install_alternative, config-tools, 0, 0, 0640, /etc/specific/dhcp_gateway.conf)
endif

ifdef PTXCONF_CT_CONFIG_MODBUS
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_modbus);
endif

ifdef PTXCONF_CT_CONFIG_MOUSEPOINTER
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_mousepointer);
endif

#ifdef PTXCONF_CT_CONFIG_PLC
#	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_plc);
#endif

ifdef PTXCONF_CT_CONFIG_PORT
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_port);
endif
...
ifdef PTXCONF_CT_CONFIG_TIMEZONE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_timezone);
endif
...
ifdef PTXCONF_CT_CONFIG_TOUCH_PARAMETER
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_touch_parameter);
endif
...
ifdef PTXCONF_CT_COPY_FIRMWARE_IMAGE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/copy_firmware_image);
endif

ifdef PTXCONF_CT_GET_MIN_SD_CARD_SIZE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_min_sd_card_size);
endif
...
ifdef PTXCONF_CT_EDIT_DNS_SERVER_DHCP
	...
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/edit_dns_server);
endif
endif

ifdef PTXCONF_CT_FORMAT_MEDIUM
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/format_medium);
endif
...
ifdef PTXCONF_CT_FIRMWARE_BACKUP
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/firmware_backup);
	@$(call install_alternative, config-tools, 0, 0, 0755, /usr/sbin/save_partition.sh)
endif

ifdef PTXCONF_CT_FIRMWARE_RESTORE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/firmware_restore);
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/firmware_restore_admin);
	@$(call install_alternative, config-tools, 0, 0, 0755, /etc/config-tools/firmware_restore_status);
	@$(call install_alternative, config-tools, 0, 0, 0755, /usr/sbin/restore_partition.sh)
endif

ifdef PTXCONF_CT_GET_PLC_CONFIG
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_plc_config);
endif

ifdef PTXCONF_CT_GET_TOUCHSCREEN_EVENT
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_touchscreen_event);
endif

ifdef PTXCONF_CT_PBDP_CONFIG
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/pbdp_config);
endif

ifdef PTXCONF_CT_RESTART_WEBSERVER
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/restart_webserver);
...
ifdef PTXCONF_CT_LEDSERVER
	...
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_ledserver);
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_ledserver);
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_ledserver_generate);
endif

ifdef PTXCONF_CT_SETTINGS_BACKUP
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/settings_backup);
	@$(call install_copy, config-tools, 0, 0, 0755, /etc/config-tools/backup-restore);
endif

ifdef PTXCONF_CT_START_REBOOT
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/start_reboot);
endif

ifdef PTXCONF_CT_SWITCH_BOOTLOADER
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/switch_bootloader);
endif

ifdef PTXCONF_CT_TERMINATE_CODESYS
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/terminate_codesys);
endif

ifdef PTXCONF_CT_TERMINATE_PROCESS
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/terminate_process);
endif

ifdef PTXCONF_CT_TIME_FORMAT
	@$(call install_alternative, config-tools, 0, 0, 0640, /etc/config-tools/TIME_FORMAT);
endif

ifdef PTXCONF_CT_GET_WBM_DIAGLIST
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_wbm_diaglist);
endif

ifdef PTXCONF_CT_GET_LED_CONFIG
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_led_config);
endif

ifdef PTXCONF_CT_GET_WBM_UPLOADS
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_wbm_uploads);
endif
...
ifdef PTXCONF_CT_GET_USER_INFO
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_user_info);
endif

ifdef PTXCONF_CT_GET_USER_HASH
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_user_hash);
endif

ifdef PTXCONF_CT_DETERMINE_HOSTNAME
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/determine_hostname);
endif

ifdef PTXCONF_CT_GET_FIRMWARE_RESTORE_PACKAGES
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_firmware_restore_packages);
endif
...
ifdef PTXCONF_CT_GET_RUNTIME_CONFIG
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_runtime_config)
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_runtime_common)
endif
...
ifdef PTXCONF_CT_HTTPS_CIPHER_SELECTION
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_https_tls)
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_https_tls_config)
endif

ifdef PTXCONF_CT_GET_TELECONTROL_STATE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_telecontrol_state);
...
ifdef PTXCONF_CT_GET_SYSTEMINFO
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_systeminfo);
endif
...
ifdef PTXCONF_CT_GET_RTS3SCFG_VALUE
	...
	@$(call install_alternative, config-tools, 0, 0, 0644, /etc/rts3s.cfg, n);
endif
...
ifdef PTXCONF_WAGO_CUSTOM_INSTALL_IOCHECK_ON
  # Does not install /etc/config-tools/events/iocheckport if iocheck service is not available (default for PFC_ADV)
	...
	@$(call install_alternative, config-tools, 0, 0, 0644, /etc/config-tools/events/networking/README);
...
ifdef PTXCONF_CT_CONFIG_ETHERNET
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_ethernet);
endif

ifdef PTXCONF_CT_CONFIG_INTERFACES
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_interfaces);
endif
ifdef PTXCONF_CT_GET_DSA_MODE
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_dsa_mode);
endif
...
ifdef PTXCONF_CT_VPNCFG
	...
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/settings_backup_vpn);
endif


ifdef PTXCONF_CT_GET_INTERNAL_BOOT
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/get_boot_mode);
endif

ifdef PTXCONF_CT_CONF_INTERNAL_BOOT
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_boot_mode);
endif
...
ifdef PTXCONF_CT_INOTIFY
	@$(call install_alternative, config-tools, 0, 0, 0750, /etc/config-tools/config_inotify);
endif
